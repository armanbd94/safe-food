...
class DepoAdvance extends BaseModel
{
    ...
    *******************************************/
    ...
    private function get_datatable_query()
    {
        ...
        ->join('depos as de','coa.depo_id','de.id')
        ...
        if (!empty($this->_depo_id)) {
            $query->where('de.id', $this->_depo_id);
        }
    ...
    {
        return self::select('transactions.*','coa.id as coa_id','coa.code','de.id as depo_id','de.name as depo_name','de.mobile_no')
        ->join('chart_of_accounts as coa','transactions.chart_of_account_id','=','coa.id')
        ->join('depos as de','coa.depo_id','de.id')
        ->where([
            'transactions.voucher_type' => self::TYPE,
