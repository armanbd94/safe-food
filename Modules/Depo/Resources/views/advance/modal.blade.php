            ...
            <form id="store_or_update_form" method="post">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <x-form.selectbox labelName="Depo" name="depo" required="required" col="col-md-12" class="selectpicker"
                            onchange="getUpazilaList(this.value,2)">
                            @if (!$depos->isEmpty())
                            @foreach ($depos as $depo)
                            <option value="{{ $depo->id }}" data-coaid="{{ $depo->coa->id }}" data-name="{{ $depo->name }}">{{ $depo->name }}</option>
                            @endforeach
                            @endif
                        </x-form.selectbox>
