...
class SupplierAdvanceController extends BaseController
{
        ...
        if($request->ajax()){
            if(permission('supplier-advance-add') || permission('supplier-advance-edit')){
                ...
                try {
                    // dd($request->all());
                    if(empty($request->id)){
                        $result = $this->advance_add($request->amount,$request->supplier_coaid,$request->supplier_name,$request->payment_method,$request->account_id,$request->reference_number);
                        $output = $this->store_message($result, $request->id);
                    }else{
                        $result = $this->advance_update($request->id,$request->amount,$request->supplier_coaid,$request->supplier_name,$request->payment_method,$request->account_id,$request->reference_number);
                        $output = $this->store_message($result, $request->id);
                    }
                    DB::commit();
                } catch (Exception $e) {
    ...
    private function advance_add($amount, int $supplier_coa_id, string $supplier_name,int $payment_method, int $account_id, string $reference_number = null) {
        if(!empty($amount) && !empty($supplier_coa_id) && !empty($supplier_name)){
            ...
            $supplier_accledger = array(
                ...
                'description'         => 'Advance to '.$supplier_name,
                'debit'               => $amount,
                'credit'              => 0,
                'posted'              => 1,
                'approve'             => 1,
    ...
    private function advance_update(int $transaction_id, $amount, int $supplier_coa_id, string $supplier_name,int $payment_method, int $account_id, string $reference_number = null) {
        if(!empty($amount) && !empty($supplier_coa_id) && !empty($supplier_name)){
